...
// Extension entry point generated by `quack_rs::entry_point!`.
//
...
//
// The closure receives a `duckdb_connection` and registers all functions.
quack_rs::entry_point!(behavioral_init_c_api, |con| {
    // Safety: `con` is a valid duckdb_connection provided by the entry_point!
    // macro via duckdb_connect. The macro's closure runs in an unsafe context.
    ffi::register_all_raw(con);
    Ok(())
});
