...
- [quack-rs SDK](#quack-rs-sdk) (66-73)
...
66. **`AggregateFunctionSetBuilder` prevents the Session 10 function-set-name
...
67. **`FfiState::with_state_mut()` returns `Option` instead of raw pointer
...
68. **`VectorWriter::set_null()` calls `ensure_validity_writable` automatically**:
...
69. **`VectorReader::read_bool()` reads as `u8 != 0` by design**: The defensive
    boolean reading pattern from Session 14 (avoiding Rust's strict `bool`
    invariant for C API data) is now the only code path.

70. **`LogicalType` RAII prevents create/destroy imbalance**: The `Drop` impl
...
71. **`AggregateTestHarness` enables combine config-propagation testing without
...
72. **`sessionize` cannot use quack-rs (v0.3.0) due to window function limitation**:
...
73. **quack-rs `interval_to_micros()` has different month semantics**: The local
