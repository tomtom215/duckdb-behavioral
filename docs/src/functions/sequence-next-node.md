...
| Space | O(n) -- all events stored (each includes a String value) |

Note: Unlike other event-collecting functions where the `Event` struct is `Copy`
(16 bytes), `sequence_next_node` stores a `String` value per event, resulting in
higher memory usage per event.
